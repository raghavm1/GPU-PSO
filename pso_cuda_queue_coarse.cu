    ...
    while(idx < tile_size2){
        ...
        if(blockDim.x >= 1024){
            if(idx < 512 && privateBest[idx] < privateBest[idx+512]){
                ...
                for(int j=0; j<d; j++){
                    aux_pos[idx*d +j] = aux_pos[(idx+512)*d +j];
                }
            }
            __syncthreads();
        }
        if(blockDim.x >= 512){
            if(idx < 256 && privateBest[idx] < privateBest[idx+256]){
                ...
                for(int j=0; j<d; j++){
                    aux_pos[idx*d +j] = aux_pos[(idx+256)*d +j];
                }
            }            
            __syncthreads();
        }
        if(blockDim.x >= 256 ){
            if(idx < 128 && privateBest[idx] < privateBest[idx+128]){
                ...
                for(int j=0; j<d; j++){
                    aux_pos[idx*d +j] = aux_pos[(idx+128)*d +j];
                }
            }
            __syncthreads();
        }
        if(blockDim.x >= 128){
            if(idx < 64 && privateBest[idx] < privateBest[idx+64]){
                ...
                for(int j=0; j<d; j++){
                    aux_pos[idx*d +j] = aux_pos[(idx+64)*d +j];
                }
            }
            __syncthreads();
        }
        if(idx < 32){
            if(blockDim.x >= 64 && privateBest[idx] < privateBest[idx+32]){
                ...
                for(int j=0; j<d; j++){
                    aux_pos[idx*d +j] = aux_pos[(idx+32)*d +j];
                }
            }
            if(blockDim.x >= 32 && privateBest[idx] < privateBest[idx+16]){
                ...
                for(int j=0; j<d; j++){
                    aux_pos[idx*d +j] = aux_pos[(idx+16)*d +j];
                }
            }
            if(blockDim.x >= 16 && privateBest[idx] < privateBest[idx+8]){
                ...
                for(int j=0; j<d; j++){
                    aux_pos[idx*d +j] = aux_pos[(idx+8)*d +j];
                }
            }
            // printf("I am here 1\n");
            if(blockDim.x >= 8 && privateBest[idx] < privateBest[idx+4]){
                ...
                for(int j=0; j<d; j++){
                    aux_pos[idx*d +j] = aux_pos[(idx+4)*d +j];
                }
            }
            if(blockDim.x >= 4 && privateBest[idx] < privateBest[idx+2]){
                ...
                for(int j=0; j<d; j++){
                    aux_pos[idx*d +j] = aux_pos[(idx+2)*d +j];
                }
            }
            if(blockDim.x >= 2 && privateBest[idx] < privateBest[idx+1]){
                ...
                for(int j=0; j<d; j++){
                    aux_pos[idx*d +j] = aux_pos[(idx+1)*d +j];
                }
            }
        }
        if(threadIdx.x == 0){
            ...
            if(queue_num){
                ...
                for(int j=1; j<queue_num; j++){
                    ...
                    //    privateBestPosQueu[0] = privateBestPosQueue[j];
                        ...
                        // printf("Thread %d, iter %d, value: %f\n", idx, iter, privateBestQueue[0]);
                    }
                }
                // for(int k=0; k< d; k++){
...
void ParticleInitCoal(particle_Coal *p, int d, particle *gbest){
	...
	for(i=0; i<particle_cnt; i++) {
        for(j=0; j<d; j++) {
            ...
            p->pbest_pos[i*d+j] = p->position[i*d +j] = RND() * pos_range + min_pos; 
            // printf("RND() in init %f\n", RND()); 
            p->velocity[i*d+j] = RND() * max_v;
        }
        p->pbest_fit[i] = p->fitness[i] = fit(p->position + i*d, d);
        // printf("p->pbest_fit[i] in init %f\n", p->pbest_fit[i]); 
