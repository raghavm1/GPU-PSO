...
__global__ void findBestInOneBlock(double *aux, double *aux_pos, int d, double *gbest_position_d, double *gbest_fitness_d) {
    ...
    __shared__ double privateBest[100]; // assuming not more than 100 blocks are formed in move kernel
    __shared__ double privateBestIndex[100]; // assuming not more than 100 blocks are formed in move kernel
...
void ParticleInitCoal(particle_Coal *p, int d, particle *gbest){
	...
	for(i=0; i<particle_cnt; i++) {
        for(j=0; j<d; j++) {
            ...
            p->pbest_pos[i*d+j] = p->position[i*d +j] = 0.0; 
